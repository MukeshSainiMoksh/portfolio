...
export default function About({ about }: { about: Record<string, string> }) {
  ...
  const infoItems = [
    { label: "Location",   value: about.location     ?? "Mohali, Punjab, India",    accent: "var(--accent)",      rgb: "var(--accent-rgb)" },
    { label: "Email",      value: about.email        ?? "codermsaini@gmail.com",     accent: "var(--accent-soft)", rgb: "var(--accent-soft-rgb)" },
    { label: "Phone",      value: about.phone        ?? "+91 82190 05065",           accent: "var(--success)",     rgb: "var(--success-rgb)" },
    { label: "Status",     value: about.availability ?? "Open to Opportunities",     accent: "var(--accent)",      rgb: "var(--accent-rgb)" },
  ];
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="animate-slide-left" style={{ opacity: 0 }}>
            <div className="space-y-5 mb-8">
              <p style={{ color: "var(--text-2)", lineHeight: "1.8", fontFamily: "var(--font-sans)", fontSize: "15px" }}>
                {about.bio ??
              ...
              <p style={{ color: "var(--text-3)", lineHeight: "1.8", fontFamily: "var(--font-sans)", fontSize: "14px" }}>
            ...
            <div className="flex gap-4">
              {about.github_url && (
                <a href={about.github_url} target="_blank" rel="noopener noreferrer" className="btn-neural py-2.5 px-5" style={{ fontSize: "12px" }}>
              ...
              {about.linkedin_url && (
                <a href={about.linkedin_url} target="_blank" rel="noopener noreferrer"
                   ...
                   style={{ fontSize: "12px", color: "rgb(var(--accent-soft-rgb) / 0.85)", borderColor: "rgb(var(--accent-soft-rgb) / 0.35)", background: "rgb(var(--accent-soft-rgb) / 0.05)" }}>
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="animate-slide-right" style={{ opacity: 0 }}>
            <div className="grid grid-cols-2 gap-3 mb-4">
              {infoItems.map(({ label, value, accent, rgb }) => (
                <div
                  key={label}
                  className="p-4 transition-all duration-300 hover:-translate-y-0.5"
                  style={{
                    background: `rgb(${rgb} / 0.02)`,
                    border: `1px solid rgb(${rgb} / 0.1)`,
                    borderLeft: `2px solid rgb(${rgb} / 0.25)`,
                    borderRadius: "var(--r-lg)",
                  }}
                >
                  <p
                    className="mb-1.5 uppercase tracking-[2px]"
                    style={{ fontFamily: "var(--font-mono)", fontSize: "12px", color: accent }}
                  >
                    {label}
                  </p>
            ...
            <div
              ...
              style={{
                ...
                borderTop: "1px solid rgb(var(--accent-rgb) / 0.2)",
                borderRadius: "var(--r-lg)",
              }}
            >
